...
# 1. [deepchat v0.1.0](https://github.com/ThinkInAIXYZ/deepchat/releases/tag/v0.1.0)  
...
# 4. [readest v0.9.38](https://github.com/readest/readest/releases/tag/v0.9.38)  
...
# 5. [new-api v0.7.0-alpha.2](https://github.com/QuantumNous/new-api/releases/tag/v0.7.0-alpha.2)  
...
# 6. [xpipe 16.2.1](https://github.com/xpipe-io/xpipe/releases/tag/16.2.1)  
...
# 7. [Ghosten-Player v2.0.0](https://github.com/GhostenEditor/Ghosten-Player/releases/tag/v2.0.0)  
...
1.  完善搜索功能，增加分类查询  
!
2. 媒体详情页新增工作人员展示(导演、编剧、摄影...)
3. 支持读取本地nfo文件，默认关闭，设置➡️其他设置➡️NFO启用 
    - 开启后会优先读取nfo的数据
    - 如果nfo中的图片地址是本机上的绝对路径，那么图片会无法展示
    - 读取阿里云盘是会自动关闭，因为阿里云盘限并发连接数，此功能需要频繁读取nfo文件，因此会巨慢无比
    - 已测试过Jellyfin和tinyMediaManager生成的nfo，其他可能会出现不兼容的问题，可提交反馈
4. 支持给一条视频添加多条字幕
5. 支持读取emby/jellyfin的外挂字幕  
6. 支持读取阿里云盘转码后的内置字幕
7. 增加自动画中画功能，设置➡️其他设置➡️自动画中画，全屏播放状态下退至桌面会自动进入画中画  
8. 播放器手势拖动快进时增加提示信息 
9. 增加文件信息展示区域 
10. 元数据编辑功能调整(后续会增加更多可编辑的元数据)
11. 增加刮削相关配置
    - 如果nfo和tmdb同时关闭将禁用刮削功能
    - max cast 和 max crew 可以限制tmdb刮削到的演职人员的数量，以解决数据过多导致查询过慢。填写范围为`0-200`，填写`-1`则不做此限制
    !

## 🚀 Improvement

1. 优化themoviedb的刮削逻辑，更加高效
2. 应用启动性能优化，尤其是在数据量较大的情况下  
3. 提升http请求传输速度
4. 优化TV端输入框焦点切换交互

## 🐞 Bug Fixed
1. 播放器双指缩放后重置按钮无法隐藏 
2. webdav进行目录查询时部分文件权限不足会导致报错
3. emby/jellyfin播放没有回传记录  
6. IPTV 的EPG现支持gzip解压缩 

## 📣 Breaking Change
1. 由于字幕功能的改动和nfo读取的增加，对底层数据的改动很大，无法兼容旧版数据，因此更新后本地数据将会被重置


## What's Changed
* Releases/v2.0.0 alpha.0  
...
# 8. [winmerge v2.16.48.2](https://github.com/WinMerge/winmerge/releases/tag/v2.16.48.2)  
...
# 9. [LangBot v3.4.14.3](https://github.com/RockChinQ/LangBot/releases/tag/v3.4.14.3)  
...
# 10. [hp 4.0.5](https://github.com/Super1Windcloud/hp/releases/tag/4.0.5)  
...
# 14. [maple-font v7.2](https://github.com/subframe7536/maple-font/releases/tag/v7.2)  
...
# 15. [BongoCat v0.3.0](https://github.com/ayangweb/BongoCat/releases/tag/v0.3.0)  
...
# 16. [higress v2.1.2](https://github.com/alibaba/higress/releases/tag/v2.1.2)  
