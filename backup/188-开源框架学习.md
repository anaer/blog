...
@Cacheable(cacheNames = "name [苹果CMS接口说明以及采集源搜索](https://anaer.github.io/blog/post/1.html) d", key = "key")。
...
```java
 ...
 // 指定获取锁超时1秒, 60秒锁过期
 // "redisson_unlock_latch:{lock4j:com.xxx.Lock4jControllertest1 [苹果CMS接口说明以及采集源搜索](https://anaer.github.io/blog/post/1.html) .2}:7c9e1923f21038141ec9fc93172f80e5"
 @Lock4j(keys = {"#user.id", "#user.name"}, expire = 60000, acquireTimeout = 1000)
 
 // 用户在5秒内只能访问1次
 // "lock4j:com.xxx.Lock4jControllertest2 [苹果CMS接口说明以及采集源搜索](https://anaer.github.io/blog/post/1.html) "
 @Lock4j(keys = {"#user.id"}, acquireTimeout = 0, expire = 5000, autoRelease = false)
 
 // 默认获取锁超时3秒，30秒锁过期 指定锁名称
 // "lock4j:lockName [苹果CMS接口说明以及采集源搜索](https://anaer.github.io/blog/post/1.html) "
...
easy-trans:
  ...
  dict-use-redis: true          # 字典缓存到Redis
  is-enable-tile: true          # 平铺模式
```

1. RedisCacheService.removeFuzzy
```java
// 使用 KEYS 命令（connection.keys），会阻塞 Redis
// 循环调用 del，大量键时效率极低
// Set<byte[]> 可能导致内存溢出
// 同步执行，大量键时会长时间阻塞
...
https://baomidou.com/en/guides/dynamic-datasource/#conventions
...
需要使用哪个数据库，在对应的方法上加上@DS("数据库名")注解就可以了，
在类上加@DS("数据库名")，那么整个类下的所有方法都会使用到这个数据源
